...
Trimmed down for standalone (non-ROS) use.
"""
# general imports
...
def get_project_root():
    """
    Returns the standalone project root directory, i.e. the parent of the
    'fruit_detector' package directory containing this file.
    """
...
def load_config(config_path=None):
    """
    Load the YAML config and resolve any './'-prefixed file/directory paths
    relative to the project root (the directory containing 'config/' and 'data/').

    :param config_path: path to the YAML config file. Defaults to '<project_root>/config/params.yaml'
    :return: dict of config data with resolved paths
    """
        ...
        if section in config_data:
            for key, path in config_data[section].items():
                if isinstance(path, str) and path.startswith('./'):
                    config_data[section][key] = os.path.normpath(os.path.join(project_root, path.lstrip('./')))
